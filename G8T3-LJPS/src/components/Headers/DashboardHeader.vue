				...
				<a-col :span="24" :md="18" class="header-control">
					...
					<a-dropdown>
						<div>
							...
							<span style="margin-left: 5px;font-weight: bold">{{profileName}}</span>
						</div>
						<template #overlay>
							
						<a-menu>
							<a-menu-item>
								<router-link to="/sign-in" class="btn-sign-in" @click="signout">
									Sign out
								</router-link>
							</a-menu-item>
						</a-menu>
	...
	export default ({
		...
		data() {
			return {
				...
				profileName: JSON.parse(localStorage.getItem('staffInfo'))['name']
			}
		},
		methods: {
			resizeEventHandler(){
