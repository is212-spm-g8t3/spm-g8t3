...
INSERT INTO `courses_catalog` (`Course_ID`, `Course_Name`, `Course_Description`, `Course_Status`, `Course_Type`, `Course_Category`) VALUES
('AGR001', 'Planting Rice', 'Agriculture or farming is the practice of cultivating plants and livestock. This course is focused on planting rice', 'Active', 'External', 'Farming'),
...
('MGT004', 'Personal Effectiveness for Leaders', 'Learners will be able to acquire the skills and knowledge to undertake self-assessment in relation to one', 'Active', 'External', 'Management'),
...
INSERT INTO `system_role` (`Role_ID`, `Role_Name`) VALUES
(1, 'Admin'),
(2, 'User'),
(3, 'Manager'),
(4, 'Trainer');
...
ALTER TABLE `course_skills`
  ADD CONSTRAINT `course_fk2` FOREIGN KEY (`Course_ID`) REFERENCES `courses_catalog` (`Course_ID`),
  ADD CONSTRAINT `Skill_fk2` FOREIGN KEY (`Skill_ID`) REFERENCES `skill` (`Skill_ID`);

--
-- Constraints for table `job_role_skills`
--
ALTER TABLE `job_role_skills`
  ADD CONSTRAINT `Role_fk` FOREIGN KEY (`Job_Role_ID`) REFERENCES `job_role` (`Job_Role_ID`),
...
ALTER TABLE `learning_journey`
  ADD CONSTRAINT `lj_staff_fk` FOREIGN KEY (`Staff_ID`) REFERENCES `staff` (`Staff_ID`),
  ADD CONSTRAINT `role_fk2` FOREIGN KEY (`Job_Role_ID`) REFERENCES `job_role_skills` (`Job_Role_ID`);

--
-- Constraints for table `learning_journey_course`
--
ALTER TABLE `learning_journey_course`
  ADD CONSTRAINT `ljc_course_fk` FOREIGN KEY (`Course_ID`) REFERENCES `courses_catalog` (`Course_ID`),
  ADD CONSTRAINT `ljc_fk` FOREIGN KEY (`LJ_ID`,`Staff_ID`,`Skill_ID`) REFERENCES `learning_journey_skill` (`LJ_ID`, `Staff_ID`, `Skill_ID`),
  ADD CONSTRAINT `ljc_reg_fk` FOREIGN KEY (`Reg_ID`) REFERENCES `registration` (`Reg_ID`);

--
-- Constraints for table `learning_journey_skill`
--
ALTER TABLE `learning_journey_skill`
  ADD CONSTRAINT `ljs_fk` FOREIGN KEY (`LJ_ID`,`Staff_ID`) REFERENCES `learning_journey` (`LJ_ID`, `Staff_ID`),
  ADD CONSTRAINT `ljs_skill_fk` FOREIGN KEY (`Skill_ID`) REFERENCES `skill` (`Skill_ID`);

--
-- Constraints for table `registration`
--
ALTER TABLE `registration`
  ADD CONSTRAINT `course_fk` FOREIGN KEY (`Course_ID`) REFERENCES `courses_catalog` (`Course_ID`),
...
--
ALTER TABLE `staff`
  ADD CONSTRAINT `system_role_fk` FOREIGN KEY (`System_Role`) REFERENCES `system_role` (`Role_ID`) ON DELETE RESTRICT ON UPDATE CASCADE;
COMMIT;
